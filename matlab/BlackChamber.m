...
% BlackChamber   Phase 2 (black background). Ends after frameThreshold frames.
...
disp('===== INITIALIZED BLACKCHAMBER (single-reader) =====');
...
assignin('base','BlackChamber_startClockStr', ...
...
if ~isempty(currentFrame) && isnumeric(currentFrame) && ~isnan(currentFrame)
    ...
    if isnan(vr.startFrame)
        ...
        assignin('base','BlackChamber_firstFrameClock', now);
        fprintf('BlackChamber FIRST frame | %.0f | %s\n', ...
...
assignin('base','BlackChamber_endClock', now);
